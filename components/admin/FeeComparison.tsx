    ...
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">${fees.marketFee.toLocaleString()}</span>
              </div>
              
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Discount ({discount}%)</span>
                <span className="text-red-600">-${(fees.marketFee * discount / 100).toLocaleString()}</span>
              </div>
              
              <div className="border-t pt-2">
                <div className="flex justify-between items-center">
                  <span className="font-medium">Discounted Market</span>
                  <span className="text-xl font-bold">${discountedMarketFee.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3 space-y-1">
              ...
              <div className="flex justify-between text-sm">
                <span>Shell (15%)</span>
                <span>${(results.budgets.shellBudget * 0.15).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Interior (35%)</span>
                <span>${(results.budgets.interiorBudget * 0.35).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Landscape (12%)</span>
                <span>${(results.budgets.landscapeBudget * 0.12).toLocaleString()}</span>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-2">
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold">${fees.louisAmyFee.toLocaleString()}</span>
                </div>
              </div>
            </div>
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-green-50">
        <CardHeader>
          ...
          <CardDescription>
            Maximum of discounted market ({discountedMarketFee.toLocaleString()}) and Louis Amy ({fees.louisAmyFee.toLocaleString()})
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-green-100 rounded-lg">
              ...
              <span className="text-xl font-bold text-green-700">
                ${(fees.marketFee - contractPrice).toLocaleString()}
              </span>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              ...
              <div className="text-2xl font-bold">${results.options.A.investment.toLocaleString()}</div>
            ...
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <div className="text-2xl font-bold">${results.options.B.investment.toLocaleString()}</div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-2xl font-bold">${results.options.C.investment.toLocaleString()}</div>
